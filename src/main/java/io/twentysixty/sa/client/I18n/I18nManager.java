  ...
  public String get(String messageName, UUID connection, String language, ClassLoader classLoader) {
    ...
    if (!bundles.containsKey(connection)) {
      if (language != null && !language.isEmpty()) {
        try {
          currentBundle =
              ResourceBundle.getBundle(getRootFiles(), new Locale(language), classLoader);
        } catch (Exception e) {
          currentBundle =
              ResourceBundle.getBundle(getRootFiles(), new Locale(getDefaultLanguage()));
        }
      } else {
    ...
    try {
      if (getMessages().contains(messageName)) return this.getConfigValue(retval);
    } catch (Exception e) {
    }
